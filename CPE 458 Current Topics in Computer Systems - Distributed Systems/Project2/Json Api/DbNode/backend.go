...
func NewBackend(dbname string) (backend *Backend, err error) {
    ...
    log.Printf("LevelDB node opened: name --> %s", dbname)
    ...
    go func() {
        for {
            ...
            case putreq := <-backend.putch:
                ...
                log.Printf("LevelDB.Put(%s, %v)\n", putreq.key, putreq.val)
            case delreq := <-backend.delch:
                backend.db.Delete(delreq.key)
                log.Printf("LevelDB.Delete(%s)\n", delreq.key)
            case <-backend.quitch:
                close(backend.putch)
                close(backend.delch)
                close(backend.quitch)

                log.Printf("LevelDB Backend stoped")
                return
            }
        }
    }()

    log.Printf("LevelDB Backend started")
}
